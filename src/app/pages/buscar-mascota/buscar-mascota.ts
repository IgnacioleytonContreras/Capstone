...
export class BuscarMascota {
  ...
  buscar(): void {
    ...
    const local = this.buscarLocal(texto);
    this.resultados = local;
    this.loading = false;
  }
  ...
  private buscarLocal(texto: string): PetSearchResult[] {
    ...
    return rows.filter(p => {
      ...
      if (this.campo === 'dueno') {
        return (
          (p.dueno || '').toLowerCase().includes(t) ||
          (p.dueno_email || '').toLowerCase().includes(t)
        );
      }
